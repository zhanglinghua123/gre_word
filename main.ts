...
const refreshFile = (data:Data[],filePath:string,interval: number) => {
    return setInterval(() => {
...
const main = async () => {
    console.log('')
    let Continue = true
    const words = readDataFromFile('data.json')
    const timer = refreshFile(words,'data.json',60000);
    while(Continue){
    ...
    writeDataToFile(words,'data.json');
    clearInterval(timer);
}
